    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="mb-4">
                        <h2 class="h4 fw-bold">{{ $producto->titulo }}</h2>
                        <span class="badge bg-{{ $producto->categoria == 'Medicamentos' ? 'primary' : ($producto->categoria == 'Vitaminas' ? 'success' : ($producto->categoria == 'Cuidado Personal' ? 'info' : ($producto->categoria == 'Primeros Auxilios' ? 'danger' : 'warning'))) }} mb-3">{{ $producto->categoria }}</span>
                        <p class="text-muted">Creado el {{ $producto->created_at->format('d/m/Y') }} | Última actualización: {{ $producto->updated_at->format('d/m/Y H:i') }}</p>
                    </div>
        ...
        <div class="col-md-4">
            <div class="card shadow-sm">
                ...
                <div class="card-body text-center">
                    @if($producto->imagen)
                        <img src="{{ asset($producto->imagen) }}" alt="{{ $producto->titulo }}" class="img-fluid rounded" style="max-height: 300px;">
                    @else
